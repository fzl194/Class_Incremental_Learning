...
from utils.data import iCIFAR10, iCIFAR100, iImageNet100, iImageNet1000, CUB200, StanfordDogs120
...
class DataManager(object):
    ...
    def get_dataset(self, indices, source, mode, appendent=None, ret_data=False, rotate = False):
        if source == 'train':
        ...
        if rotate == False:
            ...
            if ret_data:
                return data, targets, DummyDataset_Rotate(data, targets, trsf, self.use_path)
            else:
                return DummyDataset_Rotate(data, targets, trsf, self.use_path)
    ...
    def __getitem__(self, idx):
        if self.use_path:
            try:
                ...
                image = self.trsf(pil_loader(self.images[0]))
        else:
...
class DummyDataset_Rotate(Dataset):
    def __init__(self, images, labels, trsf, use_path=False):
        assert len(images) == len(labels), 'Data size error!'
        self.images = images
        self.labels = labels
    ...
    def __getitem__(self, idx):
        ...
        return idx, images[0], images[1], images[2], images[3], label
