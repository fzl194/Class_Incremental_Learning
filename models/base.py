...
from utils.toolkit import tensor2numpy, accuracy, forgetting
...
class BaseLearner(object):
    ...
    def _construct_exemplar_unified(self, data_manager, m):
        ...
        for class_idx in range(self._known_classes):
            ...
            class_loader = DataLoader(class_dset, batch_size=batch_size, shuffle=False)
    ...
    def cal_All_Acc(self, data_manager, init_cls, increment):
        class_list = [0, init_cls]
        for i in range(self._cur_task):
            class_list.append(class_list[-1] + increment)
        now_acc = []
        for i in range(self._cur_task + 1):
            L, R = class_list[i], class_list[i + 1]
            test_dataset = data_manager.get_dataset(np.arange(L, R), source='test', mode='test')
            ...
            for i, (_, inputs, targets) in enumerate(test_loader):
                ...
                correct += (predicts.cpu() == targets).sum()
                total += len(targets)
            now_acc.append(np.around(tensor2numpy(correct) * 100 / total, decimals=2))
        self.all_acc.append(now_acc)
        logging.info("all acc = {}".format(self.all_acc))
        logging.info("forget acc = {}".format(forgetting(self.all_acc)))
